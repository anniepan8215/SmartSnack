...
def train():
    ...
    while True:
        ...
        if score > old_score:
            data.append({'image': current_img, 'act': action, 'ifend': int(0)})
            df.append({'length': len(game.snake), 'count_step': count_step})
            old_score = score
            count_step = 0
        ...
        if done:
            ...
            if len(df) >0:
                ...
                sum_df.to_csv('./data/operation_agent.csv', mode='a', index=False)
            df = []
            data = []
