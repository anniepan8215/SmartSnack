...
##########################
# Game agent playground #
##########################
# run this file to have a great time with game agent!
...
class Agent:
    ...
    def train_long_memory(self, game):
        ...
        self.trainer.train_step(states, actions, rewards, next_states, dones, game, self.n_games)
        # for state, action, reward, nexrt_state, done in mini_sample:
        #    self.trainer.train_step(state, action, reward, next_state, done)

    def train_short_memory(self, state, action, reward, next_state, done, game):
        self.trainer.train_step(state, action, reward, next_state, done, game, self.n_games)
...
def train():
    ...
    df = []
    count_step = 0
    slmodel = torch.load("./data/best_model.pt")
    agent = Agent(slmodel)
    game = SnakeGameAI()
    while True:
        ...
        if score > old_score:
            data.append({'image': current_img, 'act': action, 'ifend': int(0)}) # collect data for slmodel
            df.append({'length': len(game.snake), 'count_step': count_step}) # collect agent movement data
            old_score = score
            count_step = 0
        ...
        if done:
            ...
            with open('./data/train_data_fail.pkl', 'wb') as f:
                pickle.dump(data, f, protocol=pickle.HIGHEST_PROTOCOL) #save data
                f.close()
            if len(df) >0:
                sum_df = pd.DataFrame.from_records(df)
                sum_df.to_csv('./data/operation_agent.csv', mode='a', index=False) # save agent operations
            df = []
            data = []
