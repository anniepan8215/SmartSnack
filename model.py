...
class QTrainer:
    ...
    def train_step(self, state, action, reward, next_state, done, game):
        ...
        if len(state.shape) == 1:
            ...
            for idx in range(len(done)):
                ...
                if not done[idx]:
                    image = game.get_step_map(action[idx])
                    image = transform(image)
                    batch = torch.unsqueeze(image, 0).type(torch.double).to(device)
                    out_sl = self.slmodel(batch).to('cpu')
                    _, target_sl = torch.max(out_sl, 1)
                    PUNISHMENT = [1, -1, 0]
                    punish = PUNISHMENT[target_sl]
                    Q_new = reward[idx] + self.gamma * torch.max(self.model(next_state[idx])) + punish
